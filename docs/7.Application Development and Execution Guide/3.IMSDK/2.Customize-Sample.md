...
对现有的示例应用程序进行调试和优化，以满足特定的用例要求。本节将指导您在运行 Canonical Ubuntu 的**RUBIK Pi 3****目标设备上**使用 Qualcomm® 智能多媒体产品 (QIMP) SDK 下载、构建、自定义源代码，从而完全控制应用程序行为并实现跨运行时环境的性能优化。
...
* 根据上述解释
* 从 [**获取图片格式与尺寸**](../../4.peripherals-and-interfaces/3.usb.md#verify-usb-functions) 获取所连接 USB 摄像头支持的图像格式和大小的详细信息（通过yavta），并修改json或源代码。
